...
if [ -d "$DIAGNOSTICS" ]; then
  ...
  while IFS= read -r timeline; do
    [ -n "$timeline" ] || continue
    base="${timeline%.timeline.jsonl}"
    cp "$timeline" "$OUT_DIR/alicia-launches/$(basename "$timeline")"
    for artifact in "$base.log" "$base.log.sample.txt" "$base.log.sample-status.txt"; do
      [ -f "$artifact" ] || continue
      /usr/bin/sed -E \
        -e "s|$HOME|~|g" \
        -e 's/(-OP[[:space:]]+)\[[^]]*\]/\1[REDACTED]/g' \
        -e 's/([?&](token|code|state|oauth_state|launcher_state)=)[^&[:space:]]+/\1[REDACTED]/g' \
        "$artifact" > "$OUT_DIR/alicia-launches/$(basename "$artifact")"
    done
  done < <(/bin/ls -1t "$DIAGNOSTICS"/*.timeline.jsonl 2>/dev/null | /usr/bin/head -n 12 || true)
fi
...
PREFIX="$APP_SUPPORT/prefixes/shared"
{
  ...
  echo
  echo "Relevant game/runtime files found under drive_c (paths have home replaced with ~):"
