...
namespace
{
    ...
    const QVector<DynamicTranslationTemplate>& dynamic_translation_templates()
    {
        static const QStringList templates {
            ...
            QStringLiteral(" The runtime launch process exited with code %1%2."),
            ...
            QStringLiteral("%1 passed its macOS compatibility probe. Alicia will use the shared 64-bit runtime prefix and the supported built-in graphics path. Game-local components are verified before launch."),
            ...
            QStringLiteral("<td width=\"50%\" style=\"padding:1px 10px 2px 0; white-space:nowrap;\"><b>%1:</b> <span style=\"color:%2\">%3</span></td>"),
            QStringLiteral("<tr>%1%2</tr>"),
            QStringLiteral("[HKEY_CURRENT_USER\\%1]\n"),
            QStringLiteral("alicia-%1.log"),
            QStringLiteral("Alicia.exe was not observed within %1 seconds. The launcher ended monitoring instead of waiting forever. The launch timeline records whether the runtime host was still running or had already exited."),
            QStringLiteral("Built with Qt %1"),
            QStringLiteral("CHECKING FILES (%1/%2)"),
            QStringLiteral("Compatibility profile failed: %1"),
            QStringLiteral("Could not atomically activate managed runtime: %1"),
            QStringLiteral("Could not serialize managed runtime state: %1"),
            QStringLiteral("Could not start %1"),
            QStringLiteral("Could not write managed runtime state: %1"),
            ...
            QStringLiteral("Graphics: built-in · Prefix: 64-bit · Rosetta: %1"),
            ...
            QStringLiteral("Managed runtime active.json could not be opened: %1"),
            QStringLiteral("Managed runtime active.json is invalid: %1"),
            ...
            QStringLiteral("Runtime entrypoint '%1' is not a safe relative path."),
            QStringLiteral("Runtime entrypoint is missing or not executable: %1"),
            QStringLiteral("Runtime entrypoint path is unsafe: %1"),
            QStringLiteral("Runtime entrypoint resolves outside its installation: %1"),
            QStringLiteral("Runtime manifest could not be opened: %1"),
            QStringLiteral("Runtime manifest field '%1' cannot be empty."),
            QStringLiteral("Runtime manifest field '%1' must be a positive integer."),
            QStringLiteral("Runtime manifest field '%1' must be a string."),
            QStringLiteral("Runtime manifest field '%1' must be an integer."),
            QStringLiteral("Runtime manifest JSON is invalid: %1"),
            QStringLiteral("Runtime requires launcher contract %1, but this launcher supports %2."),
            QStringLiteral("Signed in as %1"),
            QStringLiteral("Step %1 of 3"),
            QStringLiteral("The active runtime does not provide entrypoint '%1'."),
            QStringLiteral("The authentication service did not return the security state generated by the launcher. This is a compatibility fallback and is less secure.\n\nOnly continue if the browser has just completed the login you started and this is the expected account.\n\nContinue signing in as %1?"),
            ...
            QStringLiteral("The runtime could not start the game launch process: %1"),
            QStringLiteral("The runtime failed to start: %1"),
            QStringLiteral("The runtime launch process exited and Alicia.exe was not observed within %1 seconds. The launcher stopped monitoring instead of leaving the runtime host in the background. Open the generated runtime-doctor and launch timeline reports for the first missing library or graphics error."),
            QStringLiteral("The runtime started, but Alicia.exe was never observed in the process list. The launcher stayed in Launching instead of falsely reporting Running; check the diagnostic log for the first runtime or DLL error."),
            QStringLiteral("The runtime started, but Alicia.exe was never observed in the process list. The runtime launch process exited with code %1. The launcher stayed in Launching instead of falsely reporting Running; check the diagnostic log for the first runtime or DLL error."),
            QStringLiteral("The runtime started, but Alicia.exe was never observed in the process list. The runtime launch process exited with code %1 after a crash. The launcher stayed in Launching instead of falsely reporting Running; check the diagnostic log for the first runtime or DLL error."),
            ...
            QStringLiteral("The Story of Alicia runtime was not found. Use Locate Runtime to select the custom runtime app or folder.\n\nMissing: %1."),
            QStringLiteral("Time remaining: %1"),
            QStringLiteral("timeout_ms=%1 attempts=%2 wrapper_finished=%3 wrapper_exit=%4"),
            QStringLiteral("Unknown runtime entrypoint: %1"),
            QStringLiteral("Unsupported runtime manifest schema version %1."),
