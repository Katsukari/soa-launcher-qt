...
GameInstall::GameInstall(soa::runtime::Shell* shell_, QWidget* parent) : ModalOverlay(parent), shell(shell_)
{
    refresh_game_path();
...
void GameInstall::setup_close_button()
{
    ...
    close_button->setIconSize(soa::ui::layout::install_modal::close_icon(w));
    close_button->setGeometry(soa::ui::layout::install_modal::close(w));
...
void GameInstall::setup_buttons()
{
    ...
    cancel_button->setIcon(QIcon(soa::ui::assets::button(soa::ui::assets::Button::Cancel).normal));
    const QRect cancel_rect = soa::ui::layout::install_modal::cancel_button(w);
    ...
    install_button = soa::ui::simple_utils::make_flat_button(this);
    install_button->setIcon(QIcon(soa::ui::assets::button(soa::ui::assets::Button::Install).normal));
    const QRect install_rect = soa::ui::layout::install_modal::install_button(w);
    install_button->setIconSize(install_rect.size());
    install_button->setGeometry(install_rect);
    QFont install_font = soa::ui::assets::fonts[soa::ui::assets::Font::EurostileExtraBlack];
    install_font.setPixelSize(soa::ui::layout::scaled(12, w));
    install_font.setWeight(QFont::Black);
    soa::ui::simple_utils::add_button_text(install_button, soa::ui::assets::Button::Install, QStringLiteral("INSTALL"), install_font);
    ...
    change_path_button->setStyleSheet(soa::ui::styles::link_blue_lg(w));
    change_path_button->setGeometry(soa::ui::layout::install_modal::change_path_button(w));
    ...
    connect(change_path_button, &QPushButton::clicked, this, [this]
    {
        const QString dir = QFileDialog::getExistingDirectory(
            this,
            soa::i18n::translate("Select Game Install Location"),
...
void GameInstall::paint_content(QPainter& painter)
{
    ...
    const QRect box = soa::ui::layout::install_modal::box_rect(w);
    painter.drawPixmap(box, soa::ui::assets::images[soa::ui::assets::Image::BoxGameInstall]);
    ...
    painter.drawText(soa::ui::layout::install_modal::title(w), Qt::AlignCenter,
                     soa::i18n::translate("GAME INSTALLATION"));

    QFont body_font = soa::ui::assets::fonts[soa::ui::assets::Font::Inter];
    body_font.setPixelSize(soa::ui::layout::scaled(soa::ui::layout::text::k_body, w));
    ...
    painter.drawText(
        ...
        soa::i18n::translate(installDescription));

    const QRect path_rect = soa::ui::layout::install_modal::path_field(w);
    painter.drawPixmap(path_rect, soa::ui::assets::images[soa::ui::assets::Image::InstallPath]);

    QFont caption_font = soa::ui::assets::fonts[soa::ui::assets::Font::Inter];
    caption_font.setPixelSize(soa::ui::layout::scaled(soa::ui::layout::text::k_desc, w));
    caption_font.setWeight(QFont::Normal);
    painter.setFont(caption_font);
    painter.setPen(soa::ui::colors::k_text_caption);
    painter.drawText(path_rect.adjusted(soa::ui::layout::scaled(10, w), soa::ui::layout::scaled(8, w), -soa::ui::layout::scaled(10, w), 0), Qt::AlignTop | Qt::AlignLeft, soa::i18n::translate("GAME INSTALLATION PATH"));

    QFont path_font = soa::ui::assets::fonts[soa::ui::assets::Font::EurostileBold];
    path_font.setPixelSize(soa::ui::layout::scaled(16, w));
    path_font.setWeight(QFont::ExtraBold);
    painter.setFont(path_font);
    painter.setPen(soa::ui::colors::k_text_maroon);
    const QString elided = painter.fontMetrics().elidedText(
        game_path, Qt::ElideMiddle, path_rect.width() - soa::ui::layout::scaled(28, w));
    painter.drawText(path_rect.adjusted(soa::ui::layout::scaled(14, w), soa::ui::layout::scaled(34, w), -soa::ui::layout::scaled(14, w), 0), Qt::AlignTop | Qt::AlignLeft, elided);

    QFont note_font = soa::ui::assets::fonts[soa::ui::assets::Font::Inter];
    note_font.setPixelSize(soa::ui::layout::scaled(soa::ui::layout::text::k_desc, w));
    note_font.setWeight(QFont::Medium);
    painter.setFont(note_font);
    painter.setPen(soa::ui::colors::k_text_caption);
    const QRect change_path_hit = soa::ui::layout::install_modal::change_path_button(w);
    const QRect disk_space_line = soa::ui::layout::install_modal::changepath_line(w)
        .adjusted(change_path_hit.width() + soa::ui::layout::scaled(8, w), 0, 0, 0);
    const QString disk_space_text = painter.fontMetrics().elidedText(
        soa::i18n::translate("~ 2 GB of free disk space required."),
    ...
    if (show_warning)
    {
        QFont warn_font = soa::ui::assets::fonts[soa::ui::assets::Font::Inter];
        warn_font.setPixelSize(soa::ui::layout::scaled(soa::ui::layout::text::k_desc, w));
        warn_font.setWeight(QFont::DemiBold);
        painter.setFont(warn_font);
        painter.setPen(soa::ui::colors::k_warning);
        const QString warning = QStringLiteral(
            "The game must be installed inside the Wine prefix.");
        painter.drawText(
            soa::ui::layout::install_modal::warning_line(w),
            Qt::AlignHCenter | Qt::AlignVCenter,
            soa::i18n::translate(warning));
    }
}
...
bool GameInstall::eventFilter(QObject* obj, QEvent* event)
{
    if (obj == cancel_button || obj == install_button)
    {
        ...
        const auto& install = soa::ui::assets::button(soa::ui::assets::Button::Install);
        if (obj == cancel_button)
            soa::ui::simple_utils::apply_button_state(event, cancel_button, cancel.normal, cancel.hover, cancel.clicked);
        else
            soa::ui::simple_utils::apply_button_state(event, install_button, install.normal, install.hover, install.clicked);
    }
